...
def prim(graph, start_node):
    ...
    #print("first heap",candidate)
    mst = [] # mst
    total_weight = 0 # 전체 가중치

    while candidate:# o(n)
        weight, u, v = heapq.heappop(candidate) # 가중치가 가장 적은 간선 추출 #logn 힙은 logn 추출
        #print("low priorty",weight,u,v)
        if visited[v] == 0: # 방문하지 않았다면
            visited[v] = 1 # 방문 갱신
            mst.append((u,v)) # mst 삽입
            total_weight += weight # 전체 가중치 갱신

            for edge in graph[v]: # 다음 인접 간선 탐색log
